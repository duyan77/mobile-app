...
def stripe_webhook(request):
    if request.method == 'POST':
        ...
        if event['type'] == 'charge.updated':
            ...
            send_payment_confirmation_email(api_key,email,name,transaction_id,amount,currency,receipt_url)
        return JsonResponse({'status': 'success'}, status=200)
    else:
        return JsonResponse({'error': 'Invalid request method'}, status=405)  # Nếu không phải POST, trả lỗi 405
...
def send_payment_confirmation_email(api_key, to_email, to_name, transaction_id, amount, currency, receipt_url):
    ...
    data = {
        "sender": {"name": "Công Ty An Toàn Thông Tin Của Tấn", "email": "info@antoanthongtin.online"},
